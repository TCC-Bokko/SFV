...
namespace Physics {
	...
	Vector fuerzaResultante(std::vector<Vector*> fuerzas) {
		GLfloat x, y, z;
		for (auto f : fuerzas) {
			x += f->getX();
			y += f->getY();
	...
	GLdouble distanceBetweenPoints(Punto* a, Punto* b) {
		return abs(a - b);
	}
	GLdouble distancePoint2Plane(Punto* p, Plano* pl) {
		GLdouble num = abs(
			(pl->getA() * p->getX()) + 
			(pl->getB() * p->getY()) + 
			(pl->getC() * p->getZ()) +
		...
		GLfloat den = pl->getNormal().modulo();
		return num/den;
	}
	Vector productoVectorial(Vector* u, Vector* v) {
	...
	double productoEscalar(Vector* u, Vector* v) {
	...
	double angleBetweenVectors(Vector* v, Vector* u) {
